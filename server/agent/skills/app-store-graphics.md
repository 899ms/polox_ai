...
Submit generation only after both confirmation cards are resolved. After generation, inspect results (correct screenshot on screen, front-facing large phone, consistent exterior style, readable text, no white outer borders). Follow result-evaluation: if anything looks wrong, lead with ⚠️ and ask_user before regenerating — never auto-retry under Automatic. Deliver accepted results in order with short captions. Do not claim App Store upload-pixel compliance unless an export step was actually run. `export_zip` with `preset: "original"` is optional if the user asks for a download pack; there is no App Store pixel preset yet.
