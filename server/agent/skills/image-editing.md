...
First identify the user's actual source image. If none is available, ask for an upload in chat and wait. After the image arrives, continue from this method checkpoint in the same editing request — do not restart as a vague chat questionnaire.

**Clear modification request (skip method card):** only when this turn already names the specific visual changes to apply (for example “change the hair to red”, “replace the background with a beach”, “remove the logo in the corner”). Then do **not** ask annotate vs describe. Proceed straight to generation using Text description with that request. Do not restate or re-ask the goal. Credit authorization Automatic still applies.

**Vague edit intent (method card required):** if the user only asked to edit / modify / retouch / Image to Image, or only uploaded or designated an image, without naming concrete changes — including phrases like “edit this image”, “modify this photo”, “我想修改这张图片”, or an upload with no edit instructions — you **must** call `ask_user` with question id `image_edit_method` as soon as the source image is available. Do **not** ask in plain chat what they want to change, do **not** solicit a free-form description, and do **not** invent edit categories. The method card is the only allowed next step.

On that card, offer `describe` (Text description: edit using natural-language instructions only) and `annotate` (Annotate image: place numbered points and describe the change at each point), plus Other with allow_custom. Always recommend Annotate image by setting `recommended_id: "annotate"`. The recommendation text must also recommend annotation editing, never text description. This is a recommendation only: keep both options available and respect the user’s choice. Follow the system language rules for card text. Wait for the answer; do not call a generation tool in the same turn.

If the user explicitly asked to annotate or place points, or invoked `/image-annotation-edit` / Annotated Image Edit, open Annotated editing (skip the method card). If they explicitly chose describe, use Text description.
...
Use this path only after (a) the user chose `describe` on the method card, (b) they skipped the method card, or (c) a clear modification request skipped the method card above. Never use a plain-chat “please describe the changes” message as a substitute for the method card when the edit intent is still vague.

Reuse any edit description already supplied and proceed without re-asking. If the user chose or skipped to Text description and still gave no edit description, then ask in a short plain-chat message to describe the desired changes in their own words, then wait. Do not present an editing-goal card or suggested edit categories. Resolve only still-missing required settings using single-generator. Use the source image and a natural-language editing prompt; no annotation guide is needed. If no model was selected, use GPT Image 2.5 Sunburst Image to Image (`model_gpt_image_2_5_sunburst_image_to_image`).
